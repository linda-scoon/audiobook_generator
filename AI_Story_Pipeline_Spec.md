...
- ~~Where do generated MP3 files get stored~~ → **DECIDED. See D16 below.**
...
### D16 — Audio storage on Cloudflare R2

MP3 files generated by the ElevenLabs step are stored on Cloudflare R2. The bucket is private. The website backend generates presigned URLs per user session to serve audio to the player. Free chapters get long-expiry URLs (a week). Paid chapters get short-expiry URLs (15-30 minutes). Raw MP3 served — no HLS streaming for now (can be added later if audio piracy becomes a real problem).

Why R2: zero egress fees (critical for audio streaming), $0.015/GB/month storage, generous free tier (10 GB storage + 10M reads/month). Total project storage estimate across all 10 stories ≈ 6 GB; cost will sit inside the free tier for the foreseeable future.

Setup requirements (one-time, before Step 3 of the pipeline):
1. Cloudflare account.
2. R2 enabled in the dashboard.
3. Bucket created (private). Suggested name: `story-audio-prod`.
4. API credentials generated with read+write permission for the bucket.
5. Account ID and bucket endpoint URL noted.

The pipeline uses these credentials to upload MP3s. The website backend uses them (or a read-only variant) to generate presigned URLs.
