# `./audio` — Audiobook workflow CLI

The main command for preparing prose into tagged audio-drama scripts and generating MP3s with ElevenLabs. Run it from the repository root via the `./audio` entrypoint; the underlying Python modules live in this folder (`audio_workflow.py`, `ai_script_preparer.py`).

For a higher-level overview of the repository, see [`../../README.md`](../../README.md). For a one-command, cost-controlled wrapper around `./audio build`, see [`../run_audio/README.md`](../run_audio/README.md). For a quick description of every script in the repo, see the top-level README.
...
Stories live under `stories/` and are addressed by their real folder slug, for example `stories/the-alpha-of-ashbrook/`.
...
To scaffold a new story folder, use the `./create_story` command — see [`../create_story_structure/README.md`](../create_story_structure/README.md).
...
- `audio_workflow.py` — argparse CLI, story/voice/build orchestration, ElevenLabs and file I/O.
- `ai_script_preparer.py` — provider-agnostic AI prose → narration script preparer.

The repository-root `./audio` entrypoint inserts this directory onto `sys.path` and calls `audio_workflow.main()`. Tests under `tests/` rely on `tests/conftest.py` to do the same.
